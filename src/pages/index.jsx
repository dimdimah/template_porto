...
const index = () => {
  return (
    <Layout className="container">
      <Navbar />
      <p className="my-5 text-lg font-semibold text-slate-600">
        {Header.description}
      </p>
      <div>
        ...
        {QuickIntro.map((quickIntro) => (
          <div
            key={quickIntro.id}
            className="my-2 text-xl font-semibold text-slate-600"
          >
            <span className="flex items-start gap-4">
              <ApplePodcastsLogo size={32} weight="duotone" />
              {quickIntro.value}
            </span>
          </div>
        ))}
      ...
      <div className="">
        <h1 className="mb-5 text-2xl font-bold">Tool I use</h1>
        <Label />
      </div>
    </Layout>
  );
